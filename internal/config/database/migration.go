...
func runMigrations(db *gorm.DB) error {
	...
	if err != nil {
		return errors.Error{Context: migrationContext, Message: "Could not read migration directory", Err: err}
	}
...
func runMigration(migration *Migration, db *gorm.DB) error {
	...
	tx := db.Begin()
	if tx.Error != nil {
		return errors.Error{Context: migrationContext, Message: fmt.Sprintf("Error initiating transaction"), Err: tx.Error}
	}
...
func readMigrationFile(fileName string) (string, error) {
	...
	if err != nil {
		return "", errors.Error{Context: migrationContext, Message: "Could not read migration file", Err: err}
	}
...
func fileNameToFileID(fileName string) (string, error) {
	if fileName == "" {
		return "", errors.Error{Context: migrationContext, Message: "Migration file name can't be empty", Err: nil}
	}

	fileNameSplit := strings.Split(fileName, "_")
	if len(fileNameSplit) < 2 {
		return "", errors.Error{Context: migrationContext, Message: "Migration file name must be formatted like '${date}_${migration_action}' ", Err: nil}
	}

	_, err := strconv.Atoi(fileNameSplit[0])
	if err != nil {
		return "", errors.Error{Context: migrationContext, Message: "Migration file name must contain only numbers before the first underscore", Err: nil}
	}
